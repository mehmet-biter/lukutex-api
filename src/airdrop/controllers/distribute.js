...
exports.distribute = async(req, res, next) => {
    try {
        const deposites = await DepositesModel.fetch_all();
        const member_ids = deposites[0].map(deposit => deposit.member_id);
        const members = await MembersModel.fetch_all();

        const deposit_members = [...members[0].filter(member => member_ids.includes(member.id))].map(member => { return {...member, deposit: 'YES' } });
        const nondeposit_members = [...members[0].filter(member => !member_ids.includes(member.id))].map(member => { return {...member, deposit: 'NO' } });

        res.status(200).json(
            [...deposit_members, ...nondeposit_members]
        )
    } catch (error) {
